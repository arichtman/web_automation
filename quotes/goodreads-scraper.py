    ...
    def download_quotes_from_page(tag, page):
        ...
        def extract_quote_dict(quote_element):
            ...
            def extract_quote(quote_element):
                try:
                    quote = quote_element.find(
                        'div', {'class': 'quoteText'}).get_text("|", strip=True)
            ...
            def extract_author(quote_element):
                try:
                    author = quote_element.find(
                        'span', {'class': 'authorOrTitle'}).get_text()
                    author = author.strip()
            ...
            def extract_source(quote_element):
                try:
                    source = quote_element.find(
                        'a', {'class': 'authorOrTitle'}).get_text()
                    return source
                except:
                    return None
            ...
            def extract_tags(quote_element):
                try:
                    tags = quote_element.find(
                        'div', {'class': 'greyText smallText left'}).get_text(strip=True)
            ...
            def extract_likes(quote_element):
                try:
                    likes = quote_element.find(
                        'a', {'class': 'smallText', 'title': 'View this quote'}).get_text(strip=True)
    ...
    def download_all_pages(tag, max_pages, max_quotes):
        ...
        while p <= max_pages:
            ...
            if len(results) >= max_quotes:
                print(
                    f'Hit quote maximum ({max_quotes}) on page {p}.\nDiscontinuing search.')
...
if __name__ == '__main__':
    tag = args['tag'] if args['tag'] != None else input(
        'Provide tag to search quotes for: ')
    mp = args['max_pages']
    mq = args['max_quotes']
